...
class _SubSection:
    ...
    def render(self, section: Section, asset_rel_prefix: str) -> str:
        return (
            ...
            f'<span class="sub-num">{html.escape(self.number)}</span> '
...
class _Figure:
    ...
    def render(self, section: Section, asset_rel_prefix: str) -> str:
        ...
        if self.scale is not None:
            ...
            style = f"height:{height}; width:auto; display:block;"
        else:
...
class _Code:
    ...
    def render(self, section: Section, asset_rel_prefix: str) -> str:
        ...
        # Use a Quarto/Pandoc fenced code block so the rendered HTML gets
        # syntax highlighting.  _build_qmd splits on the delimiter comments so
        # the fenced block can live outside the surrounding raw-HTML block.
        return (
            f'  <details class="code-collapsible"{open_attr}>\n'
            f'    <summary><span class="code-lang">{lang}</span>'
            f'<span class="code-toggle"></span></summary>\n'
            f"{Report._CODE_BLOCK_START}\n"
            f"````{self.language}\n"
            f"{self.code}\n"
            f"````\n"
            f"{Report._CODE_BLOCK_END}\n"
            f"  </details>"
        )
...
class Section:
    ...
    def add_subsection(self, title: str, anchor: str | None = None) -> Section:
        """Add a numbered subsection heading with an anchor for the sidebar."""
        ...
        section_num = str(int(self.number)) if self.number.isdigit() else self.number
        label = f"{section_num}.{self._subsection_counter}"
        self.items.append(_SubSection(title=title, anchor=anchor, number=label))
        return self
...
class Report:
    ...
    _DEFAULT_FONT_WEIGHTS: dict[str, str] = {
        "title": "300",
        "section_title": "600",
        "subtitle": "400",
        "text": "400",
    ...
    _DEFAULT_CAPTION_COLORS: dict[str, str] = {
        "figure_caption": "#777777",
        "table_caption": "#777777",
    }

    # Delimiters used to break the raw-HTML wrapper around syntax-highlighted
    # code blocks generated by _Code.render().
    _CODE_BLOCK_START = "<!-- BIOINF_CODE_BLOCK_START -->"
    _CODE_BLOCK_END = "<!-- BIOINF_CODE_BLOCK_END -->"
    ...
    def __init__(
        ...
        caption_colors: dict[str, str] | None = None,
        page_width: str = "",
        ...
        self._font_weights = self._validate_font_weights(font_weights)
        self._caption_colors = self._validate_caption_colors(caption_colors)
    ...
    @property
    def font_weights(self) -> dict[str, str]:
        return dict(self._font_weights)

    @font_weights.setter
    def font_weights(self, value: dict[str, str] | None) -> None:
        self._font_weights = self._validate_font_weights(value)

    @classmethod
    def _validate_font_weights(cls, font_weights: dict[str, str] | None) -> dict[str, str]:
        weights = dict(cls._DEFAULT_FONT_WEIGHTS)
        if font_weights is None:
            return weights
        if not isinstance(font_weights, dict):
            raise TypeError(f"font_weights must be a dict, got {type(font_weights).__name__}")
        valid = set(cls._DEFAULT_FONT_WEIGHTS)
        for key, value in font_weights.items():
            if key not in valid:
                raise ValueError(f"font_weights key must be one of {sorted(valid)}, got {key!r}")
            if not isinstance(value, str):
                raise TypeError(
                    f"font_weights value for {key!r} must be a string, got {type(value).__name__}"
                )
            weights[key] = value
        return weights

    @property
    def caption_colors(self) -> dict[str, str]:
        return dict(self._caption_colors)

    @caption_colors.setter
    def caption_colors(self, value: dict[str, str] | None) -> None:
        self._caption_colors = self._validate_caption_colors(value)

    @classmethod
    def _validate_caption_colors(cls, caption_colors: dict[str, str] | None) -> dict[str, str]:
        colors = dict(cls._DEFAULT_CAPTION_COLORS)
        if caption_colors is None:
            return colors
        if not isinstance(caption_colors, dict):
            raise TypeError(f"caption_colors must be a dict, got {type(caption_colors).__name__}")
        valid = set(cls._DEFAULT_CAPTION_COLORS)
        for key, value in caption_colors.items():
            if key not in valid:
                raise ValueError(f"caption_colors key must be one of {sorted(valid)}, got {key!r}")
            if not isinstance(value, str):
                raise TypeError(
                    f"caption_colors value for {key!r} must be a string, got {type(value).__name__}"
                )
            colors[key] = value
        return colors
    ...
    def _font_css(self) -> str:
        ...
        weight_mapping = {
            "title": "--fw-title",
            "section_title": "--fw-section-title",
            "subtitle": "--fw-subtitle",
            "text": "--fw-text",
            "figure_caption": "--fw-figure-caption",
            "table_caption": "--fw-table-caption",
        }
        color_mapping = {
            "figure_caption": "--color-figure-caption",
            "table_caption": "--color-table-caption",
        }
        lines = ["<style>", ":root {"]
        for key, var in size_mapping.items():
            lines.append(f"  {var}: {self.font_sizes[key]};")
        for key, var in weight_mapping.items():
            lines.append(f"  {var}: {self.font_weights[key]};")
        for key, var in color_mapping.items():
            lines.append(f"  {var}: {self.caption_colors[key]};")
        lines.extend(["}", "</style>"])
        return "\n".join(lines)
    ...
    def _copy_template_files(self, out: Path) -> None:
        ...
        if styles.exists():
            extra = "\n".join([self._font_css(), self._layout_css()])
            styles.write_text(
...
"""
    ...
    @classmethod
    def _split_html_and_code(cls, rendered: str) -> list[tuple[str, str]]:
        """Split a rendered section into alternating HTML and markdown chunks.

        _Code.render() wraps fenced code blocks with sentinel comments.  This
        lets _build_qmd() keep the surrounding HTML inside raw `{=html}` blocks
        while leaving the fenced code block outside, so Quarto/Pandoc applies
        syntax highlighting.
        """
        chunks: list[tuple[str, str]] = []
        current = rendered
        start_len = len(cls._CODE_BLOCK_START)
        end_len = len(cls._CODE_BLOCK_END)
        while True:
            s = current.find(cls._CODE_BLOCK_START)
            if s == -1:
                if current.strip():
                    chunks.append(("html", current))
                break
            prefix = current[:s]
            if prefix.strip():
                chunks.append(("html", prefix))
            e = current.find(cls._CODE_BLOCK_END, s + start_len)
            if e == -1:
                # Malformed: treat remainder as HTML to avoid losing content.
                chunks.append(("html", current[s:]))
                break
            code = current[s + start_len : e].strip("\n")
            if code:
                chunks.append(("code", code))
            current = current[e + end_len :]
        return chunks
    ...
    def _build_qmd(self, assets_dir: str) -> str:
        ...
        parts.append("``````")
        for section in self._sections:
            for kind, chunk in self._split_html_and_code(
                section._render(asset_rel_prefix=f"{assets_dir}/")
            ):
                if kind == "html":
                    parts.append("``````{=html}")
                    parts.append(chunk)
